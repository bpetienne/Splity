...
class Trip:  Codable {
    ...
    struct CloudKey {
        ...
        static let mainDestination = "MainDestination"
        ...
        static let travellerIds = "travellerIds"
        static let costIds = "costIds"
...
//    }
   ...
   static func jsonToTrip(jsonData: Data) -> Trip?{
    
    do {
        let decoded = try JSONDecoder().decode(Trip.self, from: jsonData)
        return decoded
   ...
   static func loadTripFromJson(tripId: String) -> Trip?  {
    if let jsonData = read(fromURL: Trip.getJsonArchiveURL(tripId)) {
            return Trip.jsonToTrip(jsonData: jsonData)
        }
        return nil
    }
    ...
    func saveTripInJson() {
        if let data = tripToJson(){
            let isSuccessfulSave = NSKeyedArchiver.archiveRootObject(data, toFile: Trip.getJsonArchiveURL(self.tripId).path)
            if isSuccessfulSave {
    ...
    static func getJsonArchiveURL(_ tripUUID: String) -> URL {
        return Configuration.DocumentsDirectory.appendingPathComponent("trip_"+tripUUID+".ctrk")
    }
    ...
    func fromRecord(record: CKRecord){
        ...
        if let data = record.object(forKey: CloudKey.name) as? String {
        //if let data = record[CloudKey.name] as! String? {
            self.name = data
        }
        
        if let data = record[CloudKey.mainCurrency] as! String? {
            self.mainCurrency = data
        }
        
        if let data = record[CloudKey.mainDestination] as! String? {
            self.mainCountry = data
        }
        
        if let data = record[CloudKey.beginDate] as! Date? {
            self.beginDate = data
        }
        
        if let data = record[CloudKey.endDate] as! Date? {
            self.endDate = data
        }
        
        if let data = record[CloudKey.destinations] as! [String]? {
            self.countries = data
        }
        
        if let data = record[CloudKey.currencies] as! [String]? {
            self.currencies = data
        }
        
        if let data = record[CloudKey.note] as! String? {
            self.note = data
        }
        
        if let data = record[CloudKey.tripId] as! String? {
            self.tripId = data
        }  
    }
    ...
    {
        let record = CKRecord(recordType: "Trip", recordID: CKRecordID(recordName: tripId, zoneID: CKRecordZone(zoneName: "CostTrackerCustomZone").zoneID))
        
        if let name = name {
            record[CloudKey.name] = name as CKRecordValue
        }
        
        if let mainCurrency = mainCurrency {
            record[CloudKey.mainCurrency] = mainCurrency as CKRecordValue
        }
        if let mainCountry = mainCountry {
            record[CloudKey.mainDestination] = mainCountry as CKRecordValue
        }
        
        record[CloudKey.beginDate] = beginDate as CKRecordValue
        record[CloudKey.endDate] = endDate as CKRecordValue
        record[CloudKey.destinations] = countries as CKRecordValue
        record[CloudKey.currencies] = currencies as CKRecordValue
        record[CloudKey.note] = (note ?? "") as CKRecordValue
        record[CloudKey.tripId] = tripId as CKRecordValue
        
        record[CloudKey.costIds] = costs.map{$0.buyerId} as CKRecordValue
        record[CloudKey.travellerIds] = travellers.map{$0.key} as CKRecordValue

        
        var references = [CKReference]()
        for cost in costs {
            let costRecord = cost.toRecord(tripRecordId: record.recordID)
            let reference = CKReference(record: costRecord, action: .deleteSelf)
            references.append(reference)
        }
        record[CloudKey.costs] = references as CKRecordValue
        
        var travellersReferences = [CKReference]()
                for traveller in travellers {
                    let travellerRecord = traveller.value.toRecord()
                    let reference = CKReference(record: travellerRecord, action: .deleteSelf)
                    travellersReferences.append(reference)
                }
        
        record[CloudKey.travellers] = travellersReferences as CKRecordValue
...
//                record[CloudKey.coverPhoto] = asset
//            }
//            catch {
